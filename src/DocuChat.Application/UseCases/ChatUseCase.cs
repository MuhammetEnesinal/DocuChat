...
public class ChatUseCase : IChatUseCase
{
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        {
            var candidates = sessionWithMessages.Messages.OrderBy(m => m.CreatedAt).ToList();
            var budget = _historyTokenBudget;
            var picked = new List<(string Role, string Content)>();
            for (var i = candidates.Count - 1; i >= 0 && picked.Count < _historyMaxMessages; i--)
            {
                var m = candidates[i];
                if (m.Content.StartsWith("A�?A�?IDAKİ BELGE PARÇALARINI")) continue;
        ...
        if (chunks.Count == 0)
        {
            ...
            await _uow.SaveChangesAsync(ct);
            yield return new { type = "complete", chunks = Array.Empty<object>() };
            yield return new { type = "done" };
            yield break;
        }
        ...
        await foreach (var delta in _llm.AskStreamAsync(searchQuestion, chunks, history, ct))
        {
        ...
        if (quality.Score < 0.4)
        {
            ...
            badge = "g��� Bu cevap belgelerden derlendi — kritik kararlar için kaynağı doğrulamanızı öneririz.";
        }
        ...
        int limit, CancellationToken ct)
    {
        ...
        var popular = recentMessages
            ...
            .Where(q => !q.StartsWith("A�?A�?IDAKİ BELGE PARÇALARINI"))
