...
public sealed class RetrievalPipelineService : IRetrievalPipeline
{
        ...
        CancellationToken ct = default)
    {
        ...
        // HyDE: caller pre-compute etmemişse ve useHyde açıksa şimdi üret
        string? hydeText = null;
        if (useHyde)
        {
            if (precomputedHyde is not null)
            {
                hydeText = precomputedHyde;
            }
            else
            {
                try { hydeText = await _llm.GenerateHypotheticalDocumentAsync(question, ct); }
                catch (Exception ex) { _logger.LogWarning(ex, "[HyDE] Atlandı"); }
            }
        }

        // Embedding: boost'lu (varsa) > enriched > HyDE > ham soru (VectorSearch içinde null ise ham)
        // BM25: kısa tutulmalı; uzun metin PG tsquery stack'i taşırır → enriched (varsa) veya ham soru
        var embedText = embedBoostText ?? enrichedFromLlm ?? hydeText;
