...
public sealed class SemanticSplitter
{
    ...
    // Pencere oluşturma — universal recursive separator hierarchy.
    // Paragraf > satır > cümle > kelime sınırı denenir. Kelime ortası ASLA bölünmez.
    // Sıfır dil-spesifik kural, sıfır kısaltma listesi.
    private List<string> SplitIntoWindows(string text, int windowTokens)
    {
        return RecursiveSplitter.Split(text, windowTokens, _tokens, RecursiveSplitter.TextSeparators);
    }
