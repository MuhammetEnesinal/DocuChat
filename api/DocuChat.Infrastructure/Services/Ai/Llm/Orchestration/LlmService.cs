...
namespace DocuChat.Infrastructure.Services.Ai.Llm.Orchestration;
...
public class LlmService : ILlmService
{
    ...
    public LlmService(HttpClient http, IHttpClientFactory httpFactory, IConfiguration cfg, IFileStorage fileStorage, ITokenCounter tokenCounter, ILogger<LlmService> logger)
    {
        _cfg = cfg;
        _tokenCounter = tokenCounter;
        _logger = logger;
        _client = new MistralChatClient(http, httpFactory, cfg, fileStorage, logger);
    ...
    public async Task<string?> GenerateDocumentSummaryAsync(string sampleContent, CancellationToken ct = default)
    {
        ...
        var payload = HelperPayload(
            ...
            maxTokens: 160, temperature: 0.2f);
        ...
        try
        {
            ...
            if (summary.Length > 400) summary = summary[..400];  // çok bölümlü belge ana bölümlerini sayabilsin
