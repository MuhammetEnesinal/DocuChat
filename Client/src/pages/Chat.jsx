...
import { askQuestion, getSessions, getMessages, deleteSession } from '../services/api';
...
export default function Chat() {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (!activeSession) {
                const newSession = { id: sessionId, title: q.slice(0, 60) };
            ...
            setMessages((prev) => [...prev, { role: 'Assistant', content: 'Bir hata oluştu.', id: Date.now() + 1 }]);
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="flex flex-col w-64 flex-shrink-0" style={{ background: 'var(--surface)', borderRight: '1px solid var(--border)' }}>
                ...
                <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1">
                    {sessions.map((s) => (
                        ...
                        >
                            <span className="text-sm truncate" style={{ color: activeSession?.id === s.id ? '#e2e8f0' : '#94a3b8', maxWidth: '160px' }}>
                                {s.title || 'Sohbet'}
                            </span>
                            <button
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
                            {messages.length === 0 && (
                                <div className="flex flex-col items-center justify-center h-full text-center py-20">
                                    ...
                                    <p className="text-sm max-w-sm" style={{ color: 'var(--gray-light)' }}>
                                        Yüklü belgeler hakkında sorularınızı sorun. Sistem ilgili bölümleri bulup yanıtlayacak.
                                    </p>
                                </div>
                            )}
                            ...
                            {messages.map((msg) => (
                                <div key={msg.id} className={`flex ${msg.role === 'User' ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    <div
                                        className="max-w-2xl rounded-2xl px-4 py-3 text-sm prose-dark"
                                        style={{
